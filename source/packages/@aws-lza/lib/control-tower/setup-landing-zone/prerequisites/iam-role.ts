...
export abstract class IamRole {
   ...
   *
   * @remarks
   * IAM requires a role to have no attached managed policies and no inline policies before the role can be
   * deleted, so both are removed first. Each list is paginated in full before anything is removed, because
   * removing a policy while still paging can shift the remaining entries past the marker and leave a policy
   * attached, which then fails the delete.
   * @param client {@link IAMClient}
   * @param roleName string
   */
  private static async deleteControlTowerRole(client: IAMClient, roleName: string): Promise<void> {
    ...
    do {
      ...
      inlinePolicyNames.push(...(inlinePolicies.PolicyNames ?? []));
    ...
    for (const policyArn of attachedPolicyArns) {
      await throttlingBackOff(() =>
        client.send(new DetachRolePolicyCommand({ RoleName: roleName, PolicyArn: policyArn })),
      );
    }
