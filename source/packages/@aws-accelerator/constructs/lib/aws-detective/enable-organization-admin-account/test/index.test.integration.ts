...
import { DetectiveEnableOrganizationAdminAccountPolicyStatements } from '../../detective-organization-admin-account';
...
async function getAssertProperties(delayInMinutes?: number): Promise<AssertPropsType> {
  ...
  const client = new DetectiveClient({ credentials: integrationTest.environment.integrationAccountStsCredentials });
  return {
