...
const stack = new NetworkVpcEndpointsStack(
  app,
  `${AcceleratorStackNames[AcceleratorStage.NETWORK_VPC_ENDPOINTS]}-${env.account}-${env.region}`,
  props,
);
    ...
    cdk.assertions.Template.fromStack(stack).templateMatches({
      Resources: {
        SsmParamStackId521A78D3: {
          ...
          Properties: {
            Name: '/accelerator/AWSAccelerator-NetworkVpcEndpointsStack-333333333333-us-east-1/stack-id',
            Type: 'String',
            Value: {
              Ref: 'AWS::StackId',
            },
