...
describe('MoveAccountModule', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(awsLza, 'moveAccount')
      .mockResolvedValue(`Module ${AcceleratorModules.MOVE_ACCOUNTS} executed successfully`);
  ...
  test('Should execute successfully', async () => {
    // Setup
    const accountsCount =
      mockAccountsConfiguration.mandatoryAccounts!.length + mockAccountsConfiguration.workloadAccounts!.length;
    const expectedOutput: string[] = [];
    for (let i = 0; i < accountsCount; i++) {
      expectedOutput.push(`Module ${AcceleratorModules.MOVE_ACCOUNTS} executed successfully`);
    }
    ...
    // Execute
    const response = await MoveAccountModule.execute(param);

    // Verify
    expect(awsLza.moveAccount).toHaveBeenCalledTimes(accountsCount);
    expect(response).toBe(
      `Module "${AcceleratorModules.MOVE_ACCOUNTS}" completed successfully with status ${expectedOutput.join('\n')}`,
    );
  });
