...
describe('MoveAccountModule', () => {
  ...
  test('Should execute successfully', async () => {
    ...
    // Verify
    expect(awsLza.moveAccountsBatch).toHaveBeenCalledTimes(1);
    expect(response).toBe(`Module "${AcceleratorModules.MOVE_ACCOUNTS}" completed successfully with status ${status}`);
  });
  ...
  test('Should execute successfully when no accounts to move', async () => {
    ...
    // Verify
    expect(awsLza.moveAccountsBatch).toHaveBeenCalledTimes(0);
    expect(response).toBe(
      `Skipping module "${AcceleratorModules.INVITE_ACCOUNTS_TO_ORGANIZATIONS}" because all accounts already reside in their respective Organizational Units as defined in the configuration`,
    );
  });
