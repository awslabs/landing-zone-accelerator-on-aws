...
describe('ConfigureRootUserManagementModule', () => {
  ...
  test('Should execute successfully', async () => {
    ...
    // Execute
    const response = await ConfigureRootUserManagementModule.execute(param);

    // Verify
    expect(awsLza.configureRootUserManagment).toHaveBeenCalledTimes(1);
    expect(response).toBe(
      `Module "${AcceleratorModules.ROOT_USER_MANAGEMENT}" completed successfully with status ${status}`,
    );
  });
  ...
  test('Should execute successfully when no configuration is provided', async () => {
    ...
    const param: ModuleParams = {
      ...
      moduleRunnerParameters: {
        configs: {
          ...MOCK_CONSTANTS.configs,
          accountsConfig: mockAccountsConfig as AccountsConfig,
          globalConfig: mockGlobalConfigurationWithoutCentralRootUserManagment,
        },
        globalRegion: MOCK_CONSTANTS.globalRegion,
        resourcePrefixes: MOCK_CONSTANTS.resourcePrefixes,
        acceleratorResourceNames: MOCK_CONSTANTS.acceleratorResourceNames,
    ...
    // Verify
    expect(awsLza.configureRootUserManagment).toHaveBeenCalledTimes(0);
    expect(response).toBe(
      `Skipping module "${AcceleratorModules.ROOT_USER_MANAGEMENT}" because no configuration was provided`,
    );
  });
